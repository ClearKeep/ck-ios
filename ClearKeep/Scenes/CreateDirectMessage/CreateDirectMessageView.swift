...
struct CreateDirectMessageView: View {
	...
	@State private(set) var loadable: Loadable<CreatePeerViewModels> = .notRequested
	@State private(set) var userData: [CreatePeerUserViewModel] = []
...
private extension CreateDirectMessageView {
	var notRequestedView: some View {
		DirectMessageContentView(loadable: $loadable, userData: $userData)
	}
	...
	func loadedView(_ data: CreatePeerViewModels) -> AnyView {
		if let searchUser = data.searchUser {
			...
			return AnyView(DirectMessageContentView(loadable: $loadable, userData: .constant(userData), idUser: idUser))
		}
		...
		return AnyView(DirectMessageContentView(loadable: $loadable, userData: .constant(data.searchUser ?? [])))
	}
