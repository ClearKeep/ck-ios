	...
	func webSocket(_ webSocket: WebSocket, didReceiveMessage msg: [String: Any]) {
		...
		if let transaction = msg["transaction"] as? String,
		   let callback = janusTransactions[transaction] {
			janusTransactions = janusTransactions.filter({ $0.key != transaction })
			callback(msg)
			return
		}
		...
		repeat {
			if janus == JanusMessage.keepalive.rawValue {
				...
				if let data = plugindata["data"] as? [String: Any],
				   let pluginHandle = pluginHandlers[handleId] {
				...
				guard let pluginHandle = pluginHandlers[handleId] else {
				...
				pluginHandlers.removeValue(forKey: handleId)
