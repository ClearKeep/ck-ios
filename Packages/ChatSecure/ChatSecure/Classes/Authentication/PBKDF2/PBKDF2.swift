...
class PBKDF2 {
	...
	convenience init(passPharse: String) {
		self.init()
		self.passPharse = passPharse
	}
	
	// MARK: Encrypt & Decrypt
	func encrypt(data: [UInt8], saltHex: String) -> [UInt8]? {
		do {
			...
			let base64String: String = encData.base64EncodedString(options: .init(rawValue: 0))
			return base64String.bytes
		} catch { print(error) }
		return nil
	}
	
	func encrypt(data: [UInt8], saltHex: String, oldIv: String) -> [UInt8]? {
		do {
			let key = try PKCS5.PBKDF2(password: Array(passPharse.utf8), salt: saltHex.hexaBytes, iterations: iterationCount, keyLength: keyLength, variant: .sha1).calculate()
	...
	func decrypt(data: [UInt8], saltEncrypt: [UInt8], ivParameterSpec: [UInt8]) -> [UInt8]? {
